...
export async function GET(request: Request) {
  try {
    ...
    const queryParams = tense === 'mixed' ? [user.id, language, parentLevel] : [user.id, language, tense, parentLevel];
    ...
    const unlocked = await pool.query('SELECT COALESCE(MAX(ul.mastery_level), 0)::int AS level FROM user_lexemes ul JOIN language_lexemes l ON l.id=ul.lexeme_id WHERE ul.user_id=$1 AND l.language_code=$2', [user.id, language]);
...
export async function POST(request: Request) {
  try { const user = await requireUser(request); const { formId, answer, mode, language } = await request.json(); if (typeof formId !== 'string' || typeof answer !== 'string' || !allowedTenses.includes(mode) || !supportedLanguageCodes.includes(language)) return NextResponse.json({ error: 'Invalid forms review response.' }, { status: 400 }); if (language !== 'it') return NextResponse.json({ error: 'Conjugation practice is not available for this language yet.' }, { status: 409 }); const current = await pool.query('SELECT lc.form, lc.translation, lc.person, uc.mastery_level AS "masteryLevel" FROM user_lexeme_conjugations uc JOIN language_lexeme_conjugations lc ON lc.id=uc.conjugation_id JOIN language_lexemes l ON l.id=lc.lexeme_id WHERE uc.id=$1 AND uc.user_id=$2 AND l.language_code=$3', [formId, user.id, language]); if (!current.rows[0]) return NextResponse.json({ error: 'Review form not found.' }, { status: 404 }); const expected = mode === 'mixed' ? current.rows[0].form : mode === 'Present' ? (mode === 'Present' && request.headers.get('x-present-practice') === 'person' ? current.rows[0].person : current.rows[0].form) : current.rows[0].translation; const isCorrect = answer.trim().toLocaleLowerCase() === expected.trim().toLocaleLowerCase(); const next = calculateNextReview(isCorrect, Number(current.rows[0].masteryLevel)); await pool.query('UPDATE user_lexeme_conjugations SET mastery_level=$1, learning_level=GREATEST(learning_level,$2), next_review_at=$3, last_reviewed_at=NOW() WHERE id=$4 AND user_id=$5', [next.masteryLevel, next.masteryLevel, next.nextReviewAt, formId, user.id]); return NextResponse.json({ ...next, isCorrect }); }
