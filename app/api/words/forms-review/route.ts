...
export async function GET(request: Request) {
  try {
    ...
    if (targetLanguage !== 'it') return NextResponse.json({ error: 'Conjugation practice is not available for this language yet.' }, { status: 409 });
    ...
    return NextResponse.json({ forms: orderedForms, sourceLanguage, targetLanguage, unlockedStages: { Present: currentLevel >= 1, 'Passato prossimo': currentLevel >= 2, Imperfetto: currentLevel >= 3, Future: currentLevel >= 4, mixed: currentLevel >= 4 } });
