...
export default function LearnDashboard() {
  ...
  return <main className="shell grid-paper min-h-screen"><div className="mx-auto max-w-6xl px-5 py-6 md:px-10 md:py-9">
    ...
    <section><div className="mb-4"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#6f7e76]">Saved language pairs</p><h2 className="serif mt-2 text-3xl font-bold">Choose a language to recap.</h2></div><div className="grid gap-3 md:grid-cols-2">{pairs.length ? pairs.map(pair => { const selected = pair.sourceLanguage === activeSource && pair.targetLanguage === activeTarget; return <button type="button" key={`${pair.sourceLanguage}-${pair.targetLanguage}`} onClick={() => selectPair(pair.sourceLanguage, pair.targetLanguage)} className={`rounded-lg border p-5 text-left transition hover:-translate-y-0.5 ${selected ? 'border-2 border-[#e56f50] bg-[#fffaf1]' : 'border-[#173c3b22] bg-[#fffaf1]'}`}><div className="flex items-center justify-between"><span className="serif text-2xl font-bold">{languageName(pair.targetLanguage)} <span className="mx-2 text-[#6f7e76]">→</span> {languageName(pair.sourceLanguage)}</span><span className="text-xs font-bold text-[#e56f50]">{selected ? 'Selected' : 'Select'}</span></div><div className="mt-5 grid grid-cols-4 gap-2 text-center"><span><strong className="block text-xl">{pair.lessons}</strong><small className="text-[10px] text-[#6f7e76]">LESSONS</small></span><span><strong className="block text-xl">{pair.words}</strong><small className="text-[10px] text-[#6f7e76]">WORDS</small></span><span><strong className="block text-xl">{pair.mastered}</strong><small className="text-[10px] text-[#6f7e76]">MASTERED</small></span><span><strong className="block text-xl">{pair.due}</strong><small className="text-[10px] text-[#6f7e76]">DUE</small></span></div></button>; }) : <div className="panel p-6 text-[#6f7e76]">Add a lesson to create a recap dictionary.</div>}</div></section>
    <nav className="mt-10 grid gap-3 sm:grid-cols-3">{activePair ? <Link href="/review" className="group rounded-lg border-2 border-[#e56f50] bg-[#e56f50] p-5 text-white shadow-[4px_4px_0_#e5b94e66] transition hover:-translate-y-0.5"><span className="text-xs font-bold uppercase tracking-[.16em] text-white/80">Open activity</span><h2 className="serif mt-3 text-2xl font-bold">Recap →</h2><p className="mt-2 text-sm text-white/80">Review {languageName(activeTarget)} words.</p></Link> : <div aria-disabled="true" className="cursor-not-allowed rounded-lg border-2 border-[#173c3b22] bg-[#f5f1e9] p-5 text-[#6f7e76]"><span className="text-xs font-bold uppercase tracking-[.16em]">Unavailable</span><h2 className="serif mt-3 text-2xl font-bold">Recap</h2><p className="mt-2 text-sm">Add a lesson to create a recap dictionary.</p></div>}{activePair ? <Link href="/review/forms" className="rounded-lg border-2 border-[#173c3b] bg-[#fffaf1] p-5 transition hover:-translate-y-0.5"><span className="text-xs font-bold uppercase tracking-[.16em] text-[#6f7e76]">Open activity</span><h2 className="serif mt-3 text-2xl font-bold">Forms →</h2><p className="mt-2 text-sm text-[#6f7e76]">Practice deeper verb forms.</p></Link> : <div aria-disabled="true" className="cursor-not-allowed rounded-lg border-2 border-[#173c3b22] bg-[#f5f1e9] p-5 text-[#6f7e76]"><span className="text-xs font-bold uppercase tracking-[.16em]">Unavailable</span><h2 className="serif mt-3 text-2xl font-bold">Forms</h2><p className="mt-2 text-sm">Forms require a saved language pair.</p></div>}<Link href="/learn/new-lesson" className="rounded-lg border-2 border-[#173c3b] bg-[#fffaf1] p-5 transition hover:-translate-y-0.5"><span className="text-xs font-bold uppercase tracking-[.16em] text-[#6f7e76]">Open activity</span><h2 className="serif mt-3 text-2xl font-bold">New lesson →</h2><p className="mt-2 text-sm text-[#6f7e76]">Parse another language pair.</p></Link></nav>
