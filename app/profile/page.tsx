...
export default function ProfilePage() {
  ...
  const [maxNewCardsPerDay, setMaxNewCardsPerDay] = useState(15);
  const [maxReviewsPerDay, setMaxReviewsPerDay] = useState(150);
  ...
  useEffect(() => {
    fetch('/api/auth/me').then(async response => {
      ...
      setMaxNewCardsPerDay(data.maxNewCardsPerDay || 15);
      setMaxReviewsPerDay(data.maxReviewsPerDay || 150);
  ...
  async function saveSettings(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch('/api/profile', {
      ...
      body: JSON.stringify({ sourceLanguage: source, targetLanguage: target, timezone, maxNewCardsPerDay, maxReviewsPerDay }),
    });
    const data = await response.json();
  ...
  return <main className="shell grid-paper min-h-screen"><div className="mx-auto max-w-2xl px-5 py-6 md:px-10 md:py-9">
    ...
    <section className="panel mt-10 p-6 md:p-10"><span className="pill bg-[#b7c9ad66]">Account</span><h1 className="serif mt-5 text-4xl font-bold">Your learning context.</h1><p className="mt-4 leading-7 text-[#6f7e76]">{email}</p><p className="mt-6 text-sm text-[#6f7e76]">Your last selected recap pair is remembered here. Choose different languages whenever you parse a new lesson.</p><div className="mt-6 rounded-md border bg-[#fffaf1] p-4"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#6f7e76]">Last selected pair</p><p className="serif mt-2 text-2xl font-bold">{label(target)} <span className="mx-2 text-[#6f7e76]">→</span> {label(source)}</p></div><form onSubmit={saveSettings} className="mt-7 border-t pt-7"><h2 className="serif text-2xl font-bold">Review settings</h2><div className="mt-5 grid gap-4 sm:grid-cols-3"><label className="text-sm font-bold">Time zone<input value={timezone} onChange={event => setTimezone(event.target.value)} className="mt-2 w-full rounded-md border bg-[#fffaf1] px-3 py-2" /></label><label className="text-sm font-bold">New words/day<input type="number" min={5} max={30} value={maxNewCardsPerDay} onChange={event => setMaxNewCardsPerDay(Number(event.target.value))} className="mt-2 w-full rounded-md border bg-[#fffaf1] px-3 py-2" /></label><label className="text-sm font-bold">Reviews/day<input type="number" min={1} max={1000} value={maxReviewsPerDay} onChange={event => setMaxReviewsPerDay(Number(event.target.value))} className="mt-2 w-full rounded-md border bg-[#fffaf1] px-3 py-2" /></label></div><button type="submit" className="mt-5 rounded-md bg-[#173c3b] px-5 py-3 text-sm font-bold text-white">Save settings</button>{message && <p className="mt-3 text-sm font-bold text-[#6f7e76]">{message}</p>}</form><Link href="/learn/new-lesson" className="mt-6 inline-block rounded-md bg-[#173c3b] px-5 py-3 text-sm font-bold text-white">Parse a new lesson →</Link></section>
  </div></main>;
}
